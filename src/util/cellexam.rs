...
impl Clone for House {
    fn clone(&self) -> House {
        House { bedrooms: self.bedrooms, name: self.name.clone()}
    }
}
*/
...
impl<'a> House<'a> {
    ...
    pub fn print_house(&self) {
        println!("The house has {} bedrooms and name is {:?}", self.bedrooms, self.name);
    }

    pub fn set_name_string(&mut self, name: &'a String) {
        self.name = &name.as_str(); 
    }

    pub fn set_name_str(&mut self, name: &'a str) {
        self.name = name; 
    }
}
...
mod tests {
    ...
    #[test]
    fn unit_test_house() {
        let my_house = House { bedrooms: 2, name: &"2 houses"};
        let my_dream_house = House { bedrooms: 5, name: &"5 houses"};
    ...
    fn compare_owner_and_reference() {
        ...
        let my_house = &mut House { bedrooms: 2, name: &s1.as_str()};
